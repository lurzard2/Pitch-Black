...
public class EtherealGraphics : PBEntity.GraphicsModule
{
    ...
    public Color PrimaryColor
    {
        get
        {
            if (owner is Dreamer)
            {
                return Colors.VisibleWhite;
            }
            return Colors.PlayerPaletteBlack;
        }
    }
    public Color AccentColor
    {
        get
        {
            ...
            return Colors.ComplementaryRose;
        }
    }
    ...
    public EtherealGraphics(PBEntity owner) : base(owner)
    {
        ...
        if (owner is Stillborn)
        {
            pos = Stillborn.Pos;
        }
        scale = owner is Dreamer ? 0.5f : 0.85f;
        ...
        // Set colors
        Shader.SetGlobalColor(Plugin.ShadPropGhostSkinColor, PrimaryColor);
        Shader.SetGlobalColor(Plugin.ShadPropGhostSkinHighlightColor, AccentColor);
        Shader.SetGlobalColor(Plugin.ShadPropGhostDistortionColor, GlowColor);
    ...
    public override void Update(bool eu)
    {
        ...
        if (!spawnedWorms && owner is Dreamer)
        {
            for (int i = 0; i < DreamSpawnCaught; i++)
            {
                MiscUtils.MaterializeDreamSpawn(room, Dreamer.Pos, Enums.DreamSpawnSource.Dreamcatcher, default, true);
            }
            spawnedWorms = true;
        }
        ...
        Vector2 posFac = new Vector2(0f, Mathf.Sin(sinBob * 3.1415927f * 2f) * 18f * scale);
        if (owner is Dreamer)
        {
            newPos = Dreamer.Pos + posFac;
        }
        if (owner is Stillborn)
        {
            newPos = Stillborn.Pos + posFac;
        }
        pos = newPos;
        flipProg = Mathf.Min(1f, flipProg + flipSpeed);
        flip = Mathf.Lerp(flipFrom, flipTo, Custom.SCurve(flipProg, 0.75f));
        if (flipProg >= 1f && UnityEngine.Random.value < 0.1f)
        {
            flipFrom = flip;
            float oldFlip = Mathf.Clamp((flip + defaultFlip) / 2f + Mathf.Lerp(0.05f, 0.5f, Mathf.Pow(UnityEngine.Random.value, 2.5f)) * ((UnityEngine.Random.value < 0.5f) ? -1f : 1f), -1f, 1f);
            float newFLip = -2;
            foreach (AbstractCreature abstractCreature in room.game.Players)
            {
                ...
                if (player != null && player.room == room)
                {
                    ...
                    if (playerGraphics != null)
                    {
                        ...
                        var distance = Vector2.Distance(pos, player.mainBodyChunk.pos);
                        var playerXPos = player.mainBodyChunk.pos.x;
                        if (playerXPos < pos.x)
                        {
                            newFLip = Mathf.Clamp(distance, 0f, 0.55f);
                        }
                        if (playerXPos > pos.x)
                        {
                            newFLip = Mathf.Clamp(distance, 0f, -0.55f);
            ...
            flipTo = newFLip > -2 ? newFLip : oldFlip;
            flipProg = 0f;
    ...
    public class Rags
    {
        ...
        public void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            ...
            for (int i = 0; i < segments.Length; i++)
            {
                ...
                sLeaser.sprites[firstSprite + i].shader = Custom.rainWorld.Shaders["EtherealRag"];
    ...
    private void AddRippleRing()
    {
        if (riftRing == null)
        {
            riftRing = new RippleRing(pos, (Dreamer.behaviorModule as DreamerBehavior).afterConversationCounter, 1f, 0.5f);
            room.AddObject(riftRing);
            if (room.updateList.Contains(riftRing))
            {
    ...
    public override void InitiateSprites(RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
    {
        ...
        {
            shader = rCam.game.rainWorld.Shaders["EtherealDistortion"]
        };
        ...
        sLeaser.sprites[HeadMeshSprite].shader = rCam.game.rainWorld.Shaders["EtherealSkin"];
        ...
        for (int i = 0; i < legs.GetLength(0); i++)
        {
            ...
            sLeaser.sprites[ThightSprite(i)].shader = rCam.game.rainWorld.Shaders["EtherealSkin"];
            sLeaser.sprites[LowerLegSprite(i)] = TriangleMesh.MakeLongMesh(lowerLegSegments, false, true, "ghostPlates");
            sLeaser.sprites[LowerLegSprite(i)].shader = rCam.game.rainWorld.Shaders["EtherealSkin"];
